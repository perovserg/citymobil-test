...
export const onText = async (app, msg) => {
    ...
    if (msg.text.includes('/app/start')) await appStart(app, msg);

    if (msg.text.includes('/app/stop')) appStop(app, msg);

    if (msg.text.includes('/app/status')) appStatus(app, msg);

    if (msg.text.includes('/park/add')) parkAdd(app, msg);

    if (msg.text.includes('/park/delete'))  parkDelete(app, msg);

//    app.telegramBot.sendMessage(msg.chat.id, `I got something! ${msg.text}`);
